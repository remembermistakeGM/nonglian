<template>
	<view class="product-detail pr" :data-theme="theme()" :class="theme() || ''">
		...
		>
			<!-- 顶部固定 -->
			<view class="top-scroll-nav" :style="topBarHeight() == 0 ? '' : 'padding-top:' + topBarTop() + 'px'" :class="scrollId < 100 ? 'close' : 'open'">
				<!-- #ifdef MP-WEIXIN || APP-PLUS -->
				<view class="pr d-c-c">
					<view class="reg180" :style="topBarHeight() == 0 ? '' : 'height:' + topBarHeight() + 'px;'">
						<image @click="goback" src="/static/icon/back-jianatou.png" mode="" style="width: 48rpx;height: 48rpx;"></image>
				...
				<view class="d-b-c" style="height: 80rpx;">
					<view class="flex-1 tc f30 gray3 top-scroll-nav-item" :class="{ active: scrollId * 1 + 1 < commentTop }" @click="topId = 0">商品</view>
					<view
						class="flex-1 tc f30 gray3 top-scroll-nav-item"
						:class="{ active: scrollId * 1 + 1 < contentTop && scrollId * 1 + 1 > commentTop }"
						@click="topId = commentTop"
					>
						评价
					</view>
					<view class="flex-1 tc f30 gray3 top-scroll-nav-item" :class="{ active: scrollId * 1 + 1 > contentTop }" @click="topId = contentTop">详情</view>
				</view>
			</view>
			<!--图片-->
			...
			<template v-if="ispresale">
				...
				<view class="bg-white  m-0-20 mb20 p20 mt-down-box">
					<view class="d-e-c">
						<view class="flex-1">
							<!-- 优惠券 满减 返积分 -->
							<view class="p-10-0 line-h-50 f22 gray9 text-ellipsis" v-if="discount.give_points > 0">
								<text class="text-box">返{{ points_name() }}</text>
								商城购物返{{ points_name() }}，订单完成后最高返{{ discount.give_points }}{{ points_name() }}
							</view>
							<view class="f26 gray3 line-h-50 " v-if="discount.product_reduce.length > 0">
								<text class="manjian-box" v-for="(item, index) in discount.product_reduce" :key="index" :label="item" :value="index">
									<template v-if="item.full_type == 1">
										满{{ item.full_value }}元 减{{ item.reduce_value }}元
									</template>
									<template v-if="item.full_type == 2">
										满{{ item.full_value }}件 {{ (100 - item.reduce_value) / 10 }}折
									</template>
								</text>
							</view>
						</view>
						<view class="" v-if="discount.product_coupon.length > 0">
			...
			<!--评价-->
			<view class="product-comment" id="product-comment">
				...
				<view class="comment-list" v-if="detail.comment_data_count > 0">
					<template v-for="(item, index) in detail.commentData" :key="index">
