<template>
	<view class="fight-group-detail" v-if="!loadding">
		...
		<view class="product-info p30 d-s-c bg-white">
			<view class="product-cover"><image :src="productData.product.image[0].file_path" mode="aspectFit"></image></view>
		...
		<view class="fight-users mt20 bg-white">
			...
			<view class="d-c-c mt30 gray3">
				还差
				<text class="fb">{{ dif_people }}</text>
				人成团
			</view>
			<view class="mt20 d-c-c gray6 f34"><Countdown :config="countdownConfig" @returnVal="returnValFunc"></Countdown></view>
		</view>
...
export default {
	...
	methods: {
		...
		shareFunc() {
			...
			self.appParams.path = '/pagesPlus/assemble/fight-group-detail/fight-group-detail?' + params;
