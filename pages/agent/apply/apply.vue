	...
	<view class="apply-agent">
		...
		<template v-if="!is_applying ">
			<view class="form-wrap p30 f30">
				<view class="p30 d-c-c gray3 f36 fb">
					{{ words && words.apply && words.apply.words && words.apply.words.title.value || ''}}
				</view>
				<form @submit="formSubmit" @reset="formReset">
					...
					<view class="form-item border-b">
						...
						<input class="flex-1 ml20" name="name" type="text"  placeholder-class="grary"
							placeholder="请输入姓名" />
					</view>
					<view class="form-item border-b">
						<view class="field-name">手机号</view>
						<input class="flex-1 ml20" name="mobile" type="number"  placeholder-class="grary"
							placeholder="请输入手机" />
					</view>
					<view class="d-s-c p-20-0 f24">
						...
						<text class="red" @click="isPopup=true">
							【{{  words && words.apply && words.apply.words && words.apply.words.license.value || '' }}】
						</text>
					</view>
					<view class="d-c-c mt30">
						<button class="btn-red" form-type="submit">{{  words && words.apply && words.apply.words && words.apply.words.submit.value || '' }}</button>
					</view>
				</form>
			</view>
		</template>
		...
		<template v-if="is_applying">
			...
			<view class="p-30-0 d-c-c gray9 f26">
				{{  words && words.apply && words.apply.words && words.apply.words.wait_audit && words.apply.words.wait_audit.value || '' }}
			</view>
			<view class="p30 mt30 d-c-c">
				<button type="primary" class="btn-red" @click="gotoShop">{{ getBtnTxt }}</button>
			</view>
		</template>
