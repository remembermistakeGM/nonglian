  ...
  return common_vendor.e({
    ...
    g: common_vendor.f($data.tableData, (item, index, i0) => {
      return common_vendor.e({
        ...
        e: common_vendor.t(item.orderproduct.product_attr),
        f: common_vendor.t(item.orderproduct.total_price),
        g: item.order_master && item.order_master.order_source == 70
      }, item.order_master && item.order_master.order_source == 70 ? {
        h: common_vendor.t((item.order_master.pay_price * 1 + item.order_master.advance.pay_price * 1).toFixed(2))
      } : {
        i: common_vendor.t(item.orderproduct.total_pay_price)
      }, {
        j: item.orderproduct.is_agent == 0 && item.plate_status.value == 0 && (item.status.value == 0 || item.status.value == 10) && item.type.value == 30
      }, item.orderproduct.is_agent == 0 && item.plate_status.value == 0 && (item.status.value == 0 || item.status.value == 10) && item.type.value == 30 ? {
        k: common_vendor.o(($event) => $options.intervention(item.order_refund_id), index)
      } : {}, {
        l: item.orderproduct.is_agent == 0 && item.plate_status.value == 10 && (item.status.value == 0 || item.status.value == 10)
      }, item.orderproduct.is_agent == 0 && item.plate_status.value == 10 && (item.status.value == 0 || item.status.value == 10) ? {} : {}, {
        m: common_vendor.o(($event) => $options.gotoRefundDetail(item.order_refund_id), index),
        n: index
      });
    }),
    ...
    k: common_vendor.o((...args) => $options.scrolltoupperFunc && $options.scrolltoupperFunc(...args)),
    l: common_vendor.o((...args) => $options.scrolltolowerFunc && $options.scrolltolowerFunc(...args))
  });
}
const MiniProgramPage = /* @__PURE__ */ common_vendor._export_sfc(_sfc_main, [["render", _sfc_render], ["__file", "D:/workspace/p/nc/nc_app/pages/order/refund/index/index.vue"]]);
