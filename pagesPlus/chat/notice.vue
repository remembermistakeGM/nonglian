<template>
	<view class="im_interface">
		<scroll-view id="scrollview" scroll-y="true" :style="'height: ' + scrollviewHigh + 'px'" :scroll-with-animation="true" :scroll-top="scrollTop" @scrolltoupper="newdata">
			<view class="im_interface_content" ref="container">
				<view :class="item.msg_type == 1 ? 'im_text' : 'im_text2'" class="m-item" v-for="(item, index) in content_list" :key="index">
					<image v-if="item.type != 'product' && item.type != 'order'" class="avatar" :src="item.msg_type == 1 ? myavatarUrl : avatarUrl" mode=""></image>
					<view>
						<view class="my_date">{{ item.create_time }}</view>
						<view v-if="item.type == 0" :class="item.msg_type == 1 ? 'my_content my_text_content' : 'you_content you_text_content'">{{ item.content }}</view>
						<view v-if="item.type == 1" :class="item.msg_type == 1 ? 'my_content' : 'you_content'">
							<image @click="preview(item.content, 0)" class="cont_img" :src="item.content" mode=""></image>
						</view>
						<view v-if="item.type == 2" :class="item.msg_type == 1 ? 'my_content' : 'you_content'">
							<view class="product_txtitem">
								<view><image class="pro_img" :src="item.content.product_img" mode=""></image></view>
								<view>
						...
						<view v-if="item.type == 3" :class="item.msg_type == 1 ? 'my_content' : 'you_content'">
							<view class="o-h">
								...
								<view class="product_txtitem mb20">
									...
									<view>
										<view class="pro_txtname">{{ item.content.product_name }}</view>
										<view class="f24 gray6">共计{{ item.content.order_num }}件商品：合计￥{{ item.content.order_price }}</view>
									</view>
								</view>
								<view class="f24 gray6 ">订单号{{ item.content.order_no }}</view>
								<view class="f24 gray6 ">创建时间{{ item.content.create_time }}</view>
								<button class="orderdetail_btn" @click="gotoPage('/pages/order/order-detail?order_id=' + item.content.order_id)">查看</button>
							</view>
						</view>
						<view v-if="item.type == 'product'">
							<view class="top_pro">
								<view class="top_product" v-if="is_product">
									<view><image class="pro_img" :src="productDetail.image[0].filePath" mode=""></image></view>
									<view>
						...
						<view v-if="item.type == 'order'">
							<view class="top_pro">
								<view class="top_order" v-if="is_order">
									<view><image class="pro_img" :src="order_chat.product[0].image.file_path" mode=""></image></view>
									<view>
										<view class="pro_name mb20">你可能想咨询该订单</view>
										<view class="f24 gray6 sendord_price">共{{ order_chat.product.length }}件商品:合计￥{{ order_chat.order_price }}</view>
									</view>
...
export default {
	...
	methods: {
		...
		getAvatarUrl() {
			let self = this;
			//self.myUserId = uni.getStorageSync('chat_user_id')
			self._get(
