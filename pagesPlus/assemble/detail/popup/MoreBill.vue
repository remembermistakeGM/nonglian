<template>
	<Popup :show="isPopup" :width="width" :height="height" type="middle" :backgroundColor="backgroundColor"
		:boxShadow="boxShadow" :padding="0" @hidePopup="hidePopupFunc">
		<view class="more-bill-box">
			<view class="d-c-c p30 f30 border-b">
				正在拼团
			</view>
			<view class="list">
				<view class="item d-b-c p20 border-b" v-for="(item,index) in bill" :key="index"
					:class="{disnone:index >=10}">
					<view class="d-s-c">
						<view class="photo">
...
<style>
	...
	.more-bill-box .list {
		max-height: 660rpx;
		overflow-y: auto;
		box-shadow: 0 -10rpx 30rpx 0 inset rgba(255, 255, 255, .5);
	}
