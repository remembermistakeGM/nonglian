				...
				<view class="item" v-for="(item, index) in listData" :key="index">
						...
						<template v-if="item.order_status.value != 21">
							<view class="d-e-c">
								...
								<view class="d-e-c mt10"
									...
									<button v-if="item.pay_type.value ==20 && item.pay_source =='wx'"
										@click="wxOrder(item)" class="pay-btn theme-btn">确认收货</button>
					...
					<view class="order-bts" v-else>
						<block v-if="item.order_status.value ==10">
							<!-- 未支付取消订单 -->
							<button @click="cancelOrder(item)" class="theme-borderbtn"
								v-if="item.pay_status.value == 10">取消</button>
							<!-- 已支付取消订单 -->
							<block v-if="item.pay_status.value == 20 && item.delivery_status.value == 10">
								<button @click="cancelOrder(item)" class="theme-borderbtn">申请取消</button>
							</block>
							<!-- 订单核销码 -->
							<template
