    ...
    >
      <view class="">
		<template v-if="DataList && DataList.length > 0">
			<view class="item-wrap mb20" v-for="(v, idx) in supList" :key="index">
			<view class="d-f" v-if="v.name && v.list && v.list.length > 0">
				<text class="icon iconfont icon-dianpu1"></text>
				<view class="con_tit">{{v.name}}</view>
			</view>
          <template v-for="(item, index) in v.list" :key="index">
            <view
              :class="
                item.is_expire == 0 && item.is_use == 0
                  ? 'coupon-item coupon-item-' + item.color.text
            ...
            >
              <image
                ...
                src="/static/coupon-status1.png"
                mode=""
              ></image>
              <image
                v-if="item.is_expire == 1"
                class="coupon-disabled"
                src="/static/coupon-status2.png"
                mode=""
              ></image>
              <view class="operation d-b-c">
                ...
                <view class="right-box d-c-c d-c">
                  ...
                  <view class="f24 mb10">{{
                    item.min_price > 0
                      ? "满" + item.min_price * 1 + "元可用"
                      : "无门槛"
                  }}</view>
                  <template v-if="item.is_expire == 0 && item.is_use == 0">
                    <view
                      v-if="item.apply_range != 10"
                      class="f26 coupon-btn theme-btn"
                      @click.stop="
                        gotoPage(
                          '/pages/coupon/detail?coupon_id=' +
                            item.coupon_id +
                            '&apply_range=' +
                            item.apply_range
                        )
