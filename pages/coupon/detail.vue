<template>
	<view class="coupon_bg" v-if="!loading" :data-theme="theme()" :class="theme() || ''">
		<!-- #ifdef APP-PLUS -->
		...
		<view class="coupon-detail-top">
			<view class="top_box">
				<view>
					<text class="icon iconfont icon-dianpu1" style="color: #333333;font-size: 30rpx;"></text>
					<text class="gray3 fb f30 ml10">{{detail.supplier?detail.supplier.name:'平台通用'}}</text>
				</view>
					...
					<view :class="'coupon-item coupon-item-' + detail.color.text">
						...
						<view class="operation d-b-c w-b">
							...
							<view class="right-box d-c-c d-c">
								<view class="theme-price mb10" v-if="detail.coupon_type == 10">
									<text class="f24">￥</text>
									<text class="f52 fb">{{ detail.reduce_price * 1 }}</text>
								</view>
								<view class="mb10 theme-price" v-if="detail.coupon_type == 20">
									<text class="f52 fb">{{ detail.discount / 10 }}</text>
									<text class="f24">折</text>
								</view>
								<view class="f24 mb10">{{ detail.min_price > 0 ? '满' + detail.min_price * 1 + '元可用' : '无门槛' }}</view>
								<view v-if="detail.is_get == 0" class="f26 coupon-btn theme-btn" @click="receiveCoupon()">领取</view>
								<button v-else type="default" class="f26 coupon-btn btn-gray" @click="receiveCoupon()">已领取</button>
							</view>
						</view>
					</view>
				</view>
